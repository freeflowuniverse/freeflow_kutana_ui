export default {
    ...
    mutations: {
        refreshDevices(state, devices) {
            state.videoInputDevices = devices.filter(d => d.kind === "videoinput" && d.label);
            state.audioInputDevices = devices.filter(d => d.kind === "audioinput" && d.label);
            state.audioOutputDevices = devices.filter(d => d.kind === "audiooutput" && d.label);
        },
        ...
        setAudioDevice(state, audioDevice) {
            state.activeAudioDevice = state.audioInputDevices.filter(d => d.deviceId === audioDevice)[0];
            window.localStorage.setItem("activeAudioDevice", JSON.stringify(state.activeAudioDevice))
        },
        setVideoDevice(state, videoDevice) {
            state.activeVideoDevice = state.videoInputDevices.filter(d => d.deviceId === videoDevice)[0];
            window.localStorage.setItem("activeVideoDevice", JSON.stringify(state.activeVideoDevice))
        },
        setAudioOutputDevice(state, audioOutputDevice) {
            state.activeAudioOutputDevice = state.audioOutputDevices.filter(d => d.deviceId === audioOutputDevice)[0];
        },
    ...
    getters: {
        ...
        activeAudioDevice: state => state.activeAudioDevice || state.audioInputDevices[0],
        activeVideoDevice: state => state.activeVideoDevice || state.videoInputDevices[0],
